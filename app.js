/* SPA with Dashboard + Validation + Compact Auth Look */
...
function validateRegister({username,email,password}){
  ...
  if (s < 3) errors.password = "Password must be 8+ chars with upper/lowercase, a number, and preferably a symbol.";
  ...
  Home: (user, tab="overview") => `
    ...
    <div class="grid cols-2">
      ...
      <div class="card">
        ...
        <p class="helper">${tab === "stats" ? "Placeholder stats card." :
          tab === "settings" ? "Placeholder settings card." : "Recent actions appear here."}</p>
...
function renderLogin(){
  ...
  on(form, "submit", (e)=>{
    ...
    if (user && user.password === payload.password){
      ...
      msg.className = "alert success"; msg.textContent = "Login successful. Redirecting…";
      setTimeout(()=> location.hash="#/home", 200);
    } else {
      msg.className = "alert error"; msg.textContent = "Invalid email or password.";
    }
  });
}
...
function renderHome(){
  ...
  on(qForm,"submit",(e)=>{
    e.preventDefault();
    qErr.textContent=""; qOk.textContent="";
    const title = $("#quickTitle").value.trim();
    const prio = $("#quickPriority").value;
    if (title.length < 3){ qErr.textContent="Title must be at least 3 characters."; return; }
    if (!prio){ qErr.textContent="Please choose a priority."; return; }
    qOk.textContent="Saved (demo)."; qForm.reset();
  });
}
...
function updateChrome(){
  ...
  if (authed){ logoutBtn.classList.remove("hidden"); }
